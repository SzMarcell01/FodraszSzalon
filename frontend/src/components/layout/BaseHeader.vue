      ...
      <ul class="hidden md:flex space-x-10 text-white text-sm uppercase tracking-[0.2em] font-light">
        <li v-for="item in ['Főoldal', 'Galéria', 'Szolgáltatások', 'Csapat', 'Rólunk']" :key="item">
          <a href="#" class="relative group py-2 transition duration-500 hover:text-[#c5b4a2]">
            {{ item }}
            <span class="absolute bottom-0 left-0 w-0 h-px bg-[#c5b4a2] transition-all duration-700 ease-in-out group-hover:w-full"></span>
          </a>
        </li>
      </ul>
    </div>
    <div
        class="md:hidden fixed top-0 left-0 w-full h-screen bg-white text-black flex flex-col items-center justify-center transition-all duration-300"
        :class="isMenuOpen ? 'opacity-100 z-20' : 'opacity-0 pointer-events-none z-0'"
      >
      <ul class="flex flex-col items-center space-y-4 py-6 uppercase tracking-widest">
        <li v-for="item in ['Főoldal', 'Galéria', 'Szolgáltatások', 'Csapat', 'Rólunk']" :key="item">
          <a href="#"@click="isMenuOpen = false"class="hover:text-[#c5b4a2] transition">{{ item }}</a>
        </li>
      </ul>
    </div>
  </nav>
